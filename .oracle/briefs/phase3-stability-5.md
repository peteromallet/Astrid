...
Review the COMPLETE Wave-3-revised replacement against the COMPLETE frozen goal and North Star. Verify all seven accepted Wave 3 corrections, all retained Wave 1/Wave 2 contracts, the exact v2 field contract, catalog/lifetime/provenance/admission semantics, resource closure and exact exclusions, migration-head dependencies, operational consumers, isolated one-build wheel proof, baseline-independence receipts, all 15 criteria, and every anti-pattern. If no material change or exploration is required, answer exactly STABLE and nothing else. Otherwise return a complete replacement plan.
