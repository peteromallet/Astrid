...
Produce a COMPLETE replacement plan incorporating every accepted settled-plan Wave 6 correction while staying strictly inside the frozen agent goal and advancing the complete North Star. Preserve all still-valid Wave 1–5 contracts. Bind strict installed records/capture-time admission to the complete revision file inventory including Python without a continuous watcher or per-use tree rehash. Verify YAML table ownership claims by executing the real ordered migrations in disposable SQLite and introspecting before/after schema—no SQL parser and no duplicated DDL. Make backup and restore consume the same catalog-derived database projection. Run the generated Runaway round trip once in the authoritative full suite and extract its receipt. Freeze one exact wheel-only outside-checkout harness command and a schema-checked fifteen-row evidence matrix. Treat canonical/legacy manifest basenames as a separate implicit manifest audit outside resource classification. Retain separate stability receipts without naming a future file. Bias toward elegance and simplicity; explicitly reject every anti-pattern. State whether new exploration or [XHARD] work is required. Return the complete executable replacement plan, not a patch and not STABLE.
...
===== RAW WAVE 6 CRITIC: .oracle/findings/settled6/authority-architecture.txt =====
1. **P0 — installed admission does not bind executable Python bytes.**  
   §§III.2, III.5–7 bind an installed record to normalized manifest/provenance digests and verify only the captured manifest plus consumed resource handles. §IV.4 explicitly excludes Python from the resource grammar. Therefore, mutating an installed revision’s `.py` after snapshot capture is undetected; a lazy import can execute code that was never covered by the accepted v2 record. WP1/WP6 test manifest/resource mutation, but not Python-module mutation or an explicit immutable-revision invariant. This leaves strict v2 admission authoritative for metadata, not executable content. Resolve as an investigation: prove published revision roots are immutable, or define a narrow Python module origin/digest guarantee.

2. **P1 — migration table ownership can diverge from authoritative SQL.**  
   §II.3 makes `database.migrations[].tables` the ownership projection and says “the migration first creating a table records ownership.” §§VI.2–4 require collision checks and preserve SQL bytes, but specify no executable check that each declared table is actually created by that migration, that no SQL-created table is omitted, or that later `ALTER` claims do not invent ownership. Existing evidence shows collision registration operates on declared descriptors (`astrid/core/schema_packs/registry.py:162-180,221-294`), while migration safety checks bytes/state rather than SQL schema semantics (`migrations/runner.py:233-322`). A malformed or drifted declaration can bypass canonical table ownership while SQL remains checksum-valid.

3. **P1 — backup creation remains underspecified as a catalog consumer.**  
   E4 identifies backup creation as a raw `schema_migrations` reader without a registry (`astrid/core/backup/operations.py:1122-1172`), with restore separately rebuilding composition (`:1253-1296`). WP4 says to thread a snapshot and permits “raw database reads” after a shared probe, but does not state that backup and restore must use the same catalog-derived database projection, nor require operation receipts binding expected composition. The generic WP6 “backup/restore” check cannot prove criterion 7 without that scenario.

**North Star disposition: not yet aligned.** These are new closure gaps, not the superseded Wave 1–5 objections: executable code integrity, SQL/ownership agreement, and a fully specified operational backup/restore authority path remain unresolved.
0

===== RAW WAVE 6 CRITIC: .oracle/findings/settled6/sequence-proof.txt =====
1. **P1 — Runaway fixture is executed redundantly.**  
   `tests/packs/test_runaway_canonical_roundtrip.py::test_generated_temporary_project_round_trip` is included by the first WP6 command’s `tests/packs` directory, then run again by the exact receipt command, then included again by `python3 -m pytest`. This conflicts with §VII.2’s “final evidence command runs once” intent and WP6’s one-authoritative-owner/each-expensive-validation-once rule. The focused command needs an explicit exclusion or the receipt run must be the sole focused execution.

2. **P1 — Wheel validation commands are not operationally isolated.**  
   §IX requires an outside-checkout interpreter, `-I -P`, scrubbed path variables, and origin assertions. WP6 only supplies bare commands such as `python3 -m pytest` and `python3 -m astrid doctor`, followed by “Using the same wheel,” without an exact `InstalledArtifactHarness` invocation, working directory, interpreter path, or isolation wrapper. `ASTRID_PREBUILT_WHEEL` alone cannot prevent source-tree imports. This leaves criterion 10 and source/wheel comparison vulnerable to false source-mode proof.

3. **P2 — Criterion-15 evidence closure lacks an executable completeness contract.**  
   Criterion 15 requires every row to map to commands, artifacts, results, and independent reviewer disposition. §XI names a “fifteen-row evidence matrix,” and §XIII provides broad primary-proof labels, but neither defines the matrix schema, required non-empty fields, artifact paths, or a completeness/freshness gate. WP6’s “final matrix candidate” wording permits a manually incomplete matrix while still claiming closure. This remains an unresolved proof contract, not a prior Wave 1–5 objection.

**Disposition:** Wave 5’s capture-time admission supersession, strict records, direct-child closure, documentation/Python ownership rules, concrete receipt identity, and no-fingerprint constraints are consistently represented. No other frozen criterion or North Star anti-pattern exposes a material new defect; no new exploration or `[XHARD]` work is indicated.
0

===== RAW WAVE 6 CRITIC: .oracle/findings/settled6/simplicity-scope.txt =====
**Rank 1 — P1 defect: the resource-closure audit classifies `pack.yaml` as undeclared.**

Evidence:

- §III.1 requires every retained product pack to contain `pack.yaml`.
- §IV.1 says every remaining regular non-Python file under a bundled pack root must have exactly one of three classifications: typed runtime, supplemental, or authoring-only.
- The only exclusions are `.DS_Store` and immediate `__pycache__` bytecode.
- §III.2’s `declared_paths` includes content, rendering, migration, documentation, context, supplemental, and authoring-only paths—but not the manifest itself.
- No resource type declares the manifest as an implicit resource, and no manifest exclusion is specified.

Therefore each valid pack either fails `undeclared_file`, or must self-declare its own `pack.yaml` as a supplemental resource. The latter makes the manifest a runtime resource and creates unnecessary self-referential resource/provenance semantics. This blocks criteria **1, 3, and 10**, WP2 Boundary A, and the “zero undeclared non-Python files” gate. The plan needs one explicit rule: canonical and rejected manifest basenames are audited separately and excluded from pack-resource classification, or receive a dedicated implicit-manifest classification.

**Accepted-disposition check:** Wave 1’s catalog/schema/resource/ledger decisions, Wave 2’s executable contract/snapshot/DB/one-build decisions, Wave 3’s exclusions/provenance/lifetime decisions, Wave 4’s Runaway fixture, and Wave 5’s capture-time admission, strict records, direct-child closure, documentation, Python ownership, and concrete receipts are all represented. No superseded objection is repeated.

**Criteria disposition:** Criteria 2 and 4–9, 11–15 have explicit implementation and proof paths; criterion 12’s external-DB boundary is explicit. The single omission above remains unresolved.

**North Star:** All listed anti-patterns are explicitly rejected; this is a closure omission, not a reintroduced alternate authority. No new exploration or `[XHARD]` work is needed after clarifying the manifest audit rule.
0
