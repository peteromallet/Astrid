...
def _task(*, limits: dict[str, int] | None = None) -> dict[str, object]:
    ...
    return {"task": {
        ...
        "execution_binding": {
            ...
            "executor_incarnation": "astrid-pack-host/incarnation-1",
            "status": "claimed",
        },
...
def test_contract_target_storage_and_mounts_are_checked() -> None:
    ...
    task["task"]["execution_binding"]["actual_target"].update(
        {
            "storage": {"volume_id": "volume-1"},
            "mounts": [{"source": "volume-1", "target": "/mnt/work", "read_only": True}],
        }
...
@pytest.mark.parametrize(
    "field",
    ["actual_target", "verification", "executor_incarnation"],
)
def test_contract_requires_credential_verified_placement_projection(field: str) -> None:
    task = _task()["task"]
    task["execution_binding"].pop(field)
    with pytest.raises(HostError, match="credential-verified|executor incarnation"):
        _execution_contract(task)


def test_contract_rejects_actual_target_mismatch() -> None:
    task = _task()["task"]
    task["execution_binding"]["actual_target"]["id"] = "machine-other"
    with pytest.raises(HostError, match="actual target id"):
        _execution_contract(task)


def test_contract_rejects_resolved_target_mismatch() -> None:
    task = _task()["task"]
    task["execution_binding"]["resolved_target"]["id"] = "machine-other"
    with pytest.raises(HostError, match="resolved target"):
        _execution_contract(task)
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda verification: verification.update(method="self_report"),
        lambda verification: verification.update(verified=False),
        lambda verification: verification.update(evidence_digest="sha256:bad"),
    ],
)
def test_contract_rejects_forged_placement_verification(mutation) -> None:
    task = _task()["task"]
    mutation(task["execution_binding"]["verification"])
    with pytest.raises(HostError, match="placement verification"):
        _execution_contract(task)
