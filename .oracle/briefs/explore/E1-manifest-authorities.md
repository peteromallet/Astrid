Explore the complete pack manifest loader/validator/discovery call graph. Identify every accepted filename/form, every raw YAML/JSON parse, every reconstruction of PackDefinition, automatic local-pack creation, and all consumers that must move to one parse/normalize/validate result.
