...
Review the COMPLETE Wave-4-revised replacement against the COMPLETE frozen goal and North Star. Verify both accepted Wave 4 corrections, every prior executable contract, all 15 done criteria, every anti-pattern, and the internal accuracy of procedural/evidence references for this exact plan digest. If no material change or exploration is required, answer exactly STABLE and nothing else. Otherwise return a complete corrected replacement plan.
...
- `phase3-stability-5.md` owns any later immutable stability judgment over the plan digest.
