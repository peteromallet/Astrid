        ...
        *,
        ...
        try:
            ...
            if cancelled():
                cancelled_attempt = True
                return {"task_id": task_id, "status": "cancelled", "cancelled": True}
            def _settled_digest(value: Any) -> str | None:
                text = str(value or "")
                if not text:
                    return None
            ...
            capability_digest = _settled_digest(record.capability_digest)
            source_digest = _settled_digest(record.source_digest)
            dependency_digest = _settled_digest(record.dependency_digest)
