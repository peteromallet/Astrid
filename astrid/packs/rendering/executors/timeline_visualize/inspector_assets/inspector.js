...
function createInspector(data) {
  ...
  const tracksById=new Map((nav.tracks||[]).map(track=>[String(track.id||track.track_id),track]));
  const audioClipKinds=new Set(['audio','voiceover','music','sound']);
  const initialShot=typeof data.sampling?.options?.shot==='string'?data.sampling.options.shot:'';
  const state={start:null,end:null,search:'',shot:initialShot,track:'',density:1,mode:'all',retainCuts:Boolean(data.sampling?.include_cuts),target:null,message:'',audioOpen:false,playbackTime:0,loop:null,playing:false};
  ...
  function renderAudioStatus(){
    const audio=data.audio&&typeof data.audio==='object'?data.audio:nav.audio;
    const status=String(audio?.status||'not_analyzed').toLowerCase();
    if(status==='no_audio_stream')return {kind:'unavailable',label:'Render audio: no stream'};
    if(['analysis_error','unavailable','error'].includes(status))return {kind:'unavailable',label:'Render audio: unavailable'};
    if(['not_analyzed','unknown'].includes(status))return {kind:'unknown',label:'Render audio: not analyzed'};
    if(['ok','available','complete','analysis_complete','analyzed'].includes(status))return {kind:'available',label:'Render audio: available'};
    return {kind:'unknown',label:'Render audio: status unavailable'};
  }
  function cardAudioCues(c){
    const cues=[];
    const seen=new Set();
    for(const clip of Array.isArray(c?.clips)?c.clips:[]){
      const clipKind=String(clip?.kind||clip?.clip_kind||'').toLowerCase();
      const track=tracksById.get(String(clip?.track??clip?.track_id??''));
      const trackKind=String(track?.track_kind||track?.kind||'').toLowerCase();
      const label=clipKind==='music'||trackKind==='music'?'Music clip':audioClipKinds.has(clipKind)||trackKind==='audio'?'Audio clip':null;
      if(label&&!seen.has(label)){seen.add(label);cues.push(label);}
    }
    if(audioItems('phrase').some(p=>itemMatchesCard(p,c))&&!seen.has('Speech annotation'))cues.push('Speech annotation');
    return cues;
  }
  function inScope(c) {
  ...
  return {state,nav,fps,record,cardTarget,frameCard,inScope,visibleCards,selectedCard,select,update,step,clipVisible,decodeHash,keyboard,audioItems,audioVisible,cardAudioCues,renderAudioStatus,selectionInterval,setPlaybackTime,setLoopForSelection,clearLoop};
}
...
if(typeof document!=='undefined'){
  ...
  const shotName=c=>names(c.clips)||((c.shot_ids||[]).map(id=>app.nav.shots.find(shot=>String(shot.shot_id)===String(id))?.label||id).filter(Boolean).join(' · '))||'Unlabelled shot';
  const cardCaptionText=c=>{const source=Array.isArray(c.captions)&&c.captions.length?c.captions:Array.isArray(c.display_scripts)?c.display_scripts:(Array.isArray(c.captions)?c.captions:(Array.isArray(c.scripts)?c.scripts:[]));return source.map(s=>s.canonical_text||s.text||'').join('\n').trim();};
  const stepFrames=data.sampling.step_frames_rational[0]/data.sampling.step_frames_rational[1];
  $('title').textContent=data.provenance.timeline_name||data.provenance.timeline_id;$('provenance').textContent=`${data.provenance.project_slug} / render ${data.provenance.render_run_id}`;$('frozen').textContent='Frozen render snapshot · '+(data.provenance.metadata?.config_version?'version '+data.provenance.metadata.config_version:'exact render provenance');const captureBase=data.sampling.options.every_frames?`every ${stepFrames} frames`:`every ${(stepFrames/app.fps).toFixed(3)} seconds`;const captureSuffix=data.sampling.include_cuts?' plus explicitly included cut neighbors':data.sampling.explicit_interval?' (strict periodic samples)':' (bounded overview)';$('capture').textContent=`Captured ${captureBase}${captureSuffix} · ${data.cards.length} frames. Density can only reduce these captured samples.`;
  ...
  const capturedCuts=data.cards.some(c=>((c.sample_reasons||[]).some(r=>r.includes('cut'))));$('retain-cuts').checked=Boolean(data.sampling.include_cuts);$('retain-cuts').disabled=!capturedCuts;$('retain-cuts').title=capturedCuts?'Hide/show cut-neighbor cards already captured in this artifact.':'This artifact contains no cut-neighbor cards; regenerate with --include-cuts to capture them.';
  if(app.state.shot)$('shot').value=app.state.shot;
  ...
  function focusFirstVisibleCard(force=false){const cards=app.visibleCards();if(!cards.length||(!force&&app.selectedCard()))return false;const target=app.cardTarget(cards[0]);if(!target)return false;return app.select(target);}
  ...
  function seekToSelection(autoplay=false){const r=app.record();if(!r||!media)return;const seek=r.actions?.seek;const start=seek?seek.start[0]/seek.start[1]:r.start_seconds!==undefined?r.start_seconds:r.start_frame!==undefined?r.start_frame/app.fps:undefined;if(Number.isFinite(start)){media.currentTime=Math.max(0,start);if(autoplay)media.play().catch(()=>{});}}
  function renderGrid(){const cards=app.visibleCards(),selected=app.selectedCard();$('grid').replaceChildren();for(const c of cards){const el=node('button',undefined,'frame-card'+(selected?.id===c.id?' selected':''));el.type='button';el.setAttribute('aria-pressed',String(selected?.id===c.id));el.onclick=()=>choose(app.cardTarget(c));const header=node('div',undefined,'card-header');header.append(node('span',shotName(c),'shot-name'),node('span',c.time_label,'time'));const img=node('img');img.src=c.image;img.alt=`Composited frame ${c.frame} at ${c.time_label}`;img.loading='lazy';const spoken=cardCaptionText(c);const emptyLabel=Array.isArray(c.captions)?(c.caption_status==='same shot; no new timed text'?'':(c.caption_status||'No timed text available')):'No spoken text';const scriptBlock=node('div',undefined,'script-block');scriptBlock.append(node('span',spoken?`“${spoken}”`:emptyLabel,'caption'));const cues=app.cardAudioCues(c);const audioBlock=node('div',undefined,'audio-cues');for(const label of cues.length?cues:['No audio metadata'])audioBlock.append(node('span',label,'audio-cue'+(cues.length?'':' no-data')));audioBlock.append(node('span',app.renderAudioStatus().label,'audio-cue render-status'));el.append(header,img,scriptBlock,audioBlock);$('grid').append(el);}$('count').textContent=`${cards.length} of ${data.cards.length} captured frames · final composition`;const query=app.state.search.trim();const empty=Boolean(query&&!cards.length);$('search-status').textContent=empty?`No spoken text or annotated speech phrase matches “${query}”. Timing-only dialogue is available in Speech and audio.`:'';$('open-audio').hidden=!empty;}
  ...
  function renderAudio(){
    const audio=app.nav.audio||{},status=audio.status||'not_analyzed',speech=audio.speech||{},unavailable=(speech.unavailable||[]).length,renderStatus=app.renderAudioStatus();const baseStatus=renderStatus.kind==='unavailable'?(status==='analysis_error'?`${renderStatus.label}: ${audio.error||'decoder error'}`:`${renderStatus.label}.`):renderStatus.kind==='unknown'?`${renderStatus.label}.`:speech.status==='no_transcript'?'No speech annotation; waveform remains available.':`${renderStatus.label}; speech annotations are frozen to this render.`;$('audio-status').textContent=unavailable?`${baseStatus} ${unavailable} speech annotation${unavailable===1?' is':'s are'} unavailable because timing or coverage is incomplete.`:baseStatus;$('audio-rows').replaceChildren();const ruler=node('div',undefined,'audio-ruler ruler');ruler.append(node('span','Audio'));for(let i=0;i<=4;i++)ruler.append(node('span',((data.provenance.duration_frames/app.fps*i/4)).toFixed(1)+'s'));$('audio-rows').append(ruler);
  ...
  function renderDetail(){const card=app.selectedCard(),record=app.record();$('selection-message').textContent=app.state.message;$('detail-content').replaceChildren();$('detail-empty').hidden=!!record;$('detail-actions').hidden=!record;if(!record)return;$('detail-content').append(node('div',record.kind==='frame'?'Rendered frame':record.kind==='clip'?'Clip placement':record.kind==='track'?'Track':record.kind==='phrase'?'Speech phrase':record.kind==='gap'?'Quiet gap':record.kind==='waveform'?'Waveform bin':'Story placement','eyebrow'));if(card){const img=node('img');img.src=card.image;img.alt=`Selected composited frame ${card.frame}`;const heading=node('div',undefined,'detail-heading');heading.append(node('h2',shotName(card),'detail-shot-name'),node('p',`${card.time_label} · frame ${card.frame}`,'detail-time'));const spoken=cardCaptionText(card);const emptyLabel=Array.isArray(card.captions)?(card.caption_status==='same shot; no new timed text'?'':(card.caption_status||'No timed text available')):'No spoken text';const script=node('div',undefined,'detail-script');script.append(node('p',spoken?`“${spoken}”`:emptyLabel,'full-script'));$('detail-content').append(img,heading,script);}else if(['phrase','gap','waveform'].includes(record.kind)){$('detail-content').append(node('h2',record.label),node('p',record.kind==='gap'?`Measured low-amplitude interval; threshold ${record.threshold??'recorded in sidecar'}.`:record.kind==='phrase'?[record.canonical_text||record.label,record.recognized_text&&`Recognized: ${record.recognized_text}`,record.uncertainty!=null&&`Uncertainty: ${record.uncertainty}`].filter(Boolean).join('\n'):'Waveform sample bounds are recorded in the sidecar.','full-script'));}else $('detail-content').append(node('h2',record.label||record.clip_id||record.id||record.kind),node('p',record.kind==='track'?'Track selected. Its placements remain part of the full composition.':'No captured image in this selection. The focus command requests this exact placement.','muted'));const technical=node('details');technical.append(node('summary','Source and selection details'));technical.append(node('pre',JSON.stringify({target:app.state.target,selection:record,active_clips:card?.clips||[],scripts:card?.scripts||[],captions:card?.captions||[],video_digest:data.provenance.video_digest},null,2)));$('detail-content').append(technical);$('previous').disabled=!app.visibleCards().length;$('next').disabled=!app.visibleCards().length;}
  function render(){renderGrid();renderTracks();renderAudio();renderDetail();}
  for(const id of ['start','end','search','shot','track','density','mode','retain-cuts'])$(id).addEventListener('input',()=>{const previousShot=app.state.shot;const values={start:$('start').value===''?null:Number($('start').value),end:$('end').value===''?null:Number($('end').value),search:$('search').value,shot:$('shot').value,track:$('track').value,density:Number($('density').value),mode:$('mode').value,retainCuts:$('retain-cuts').checked};app.update(values);const shotChanged=previousShot!==app.state.shot;if(shotChanged&&app.state.shot){focusFirstVisibleCard(true);if(app.state.target)history.replaceState(null,'','#'+encodeURIComponent(app.state.target));}if(!app.state.target)history.replaceState(null,'',location.pathname+location.search);render();if(shotChanged&&app.state.shot)seekToSelection(false);});$('reset').onclick=()=>{for(const id of ['start','end','search','shot','track'])$(id).value='';app.update({start:null,end:null,search:'',shot:'',track:''});render();};
  function move(direction){app.step(direction);if(app.state.target)history.replaceState(null,'','#'+encodeURIComponent(app.state.target));render();seekToSelection(false);}$('previous').onclick=()=>move(-1);$('next').onclick=()=>move(1);document.addEventListener('keydown',e=>{if(app.keyboard(e)){e.preventDefault();move(e.key==='ArrowRight'?1:-1);}});
  ...
  const audioSection=$('audio-section');if(audioSection){try{app.state.audioOpen=localStorage.getItem('astrid.inspector.audioOpen')==='true';}catch{}audioSection.addEventListener('toggle',()=>{app.state.audioOpen=audioSection.open;try{localStorage.setItem('astrid.inspector.audioOpen',String(audioSection.open));}catch{};});}const media=$('rendered-media');if(media&&data.media?.path){media.src=data.media.path;media.hidden=false;media.setAttribute('aria-label','Verified rendered video for this pinned render');const tick=()=>{app.setPlaybackTime(media.currentTime);syncPlayhead();if(app.state.loop&&media.currentTime>=app.state.loop.end){media.currentTime=app.state.loop.start;if(app.state.playing)media.play().catch(()=>{});}};if(typeof media.requestVideoFrameCallback==='function'){const frameTick=()=>{tick();media.requestVideoFrameCallback(frameTick);};media.requestVideoFrameCallback(frameTick);}else media.addEventListener('timeupdate',tick);media.addEventListener('play',()=>app.state.playing=true);media.addEventListener('pause',()=>app.state.playing=false);media.addEventListener('loadedmetadata',()=>seekToSelection(false));if(app.state.shot)focusFirstVisibleCard();}
  $('open-audio').onclick=()=>{$('search').value='';app.update({search:'',target:null,audioOpen:true});$('track-section').open=true;$('audio-section').open=true;render();};$('loop-selection').onclick=()=>{app.setLoopForSelection();render();};$('clear-loop').onclick=()=>{app.clearLoop();render();};$('context-selection').onclick=()=>{const r=app.record();if(r?.actions?.seek&&media){const start=Math.max(0,r.actions.seek.start[0]/r.actions.seek.start[1]-.5);media.currentTime=start;media.play().catch(()=>{});}};window.addEventListener('hashchange',()=>{app.select(app.decodeHash(location.hash));render();seekToSelection(false);});if(location.hash)app.select(app.decodeHash(location.hash));if(!app.state.target&&app.state.shot)focusFirstVisibleCard();render();seekToSelection(false);syncPlayhead();
}
