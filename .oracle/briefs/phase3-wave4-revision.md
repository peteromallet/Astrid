...
Produce a COMPLETE replacement plan incorporating both accepted settled-plan Wave 4 corrections while staying strictly inside the frozen agent goal and advancing the complete North Star. Preserve every still-valid executable contract from the current plan and accepted Waves 1–3. Add only (1) a narrow immediate pre-use InstalledPackStore admission-state verification keyed by the captured revision and permission receipt, with fail-closed semantics and no rediscovery/retry/lock/new service, and (2) an unambiguous deterministic temporary-project Runaway round-trip fixture and receipt path; never restore or package the missing historical demo. Retain the separate-receipt Megado state model rather than copying current stability state into plan content. Bias toward elegance and simplicity. Explicitly reject every North Star anti-pattern. State whether new exploration or [XHARD] work is required. Return the complete executable replacement plan, not a patch and not STABLE.
...
This plan incorporates every accepted Wave 1, Wave 2, and Wave 3 correction. It removes the global snapshot fingerprint, rejected-diagnostic hashing, unstable-read retries, and broad bundled freshness verification. It introduces no cache invalidation subsystem.
...
- Runaway’s missing demo input remains a deterministic temporary-project fixture repair unless custody evidence proves it is an intentional packaged asset.
...
`InstalledPackStore` remains the typed owner for installed revision state and is extended narrowly to emit the installed candidate’s immutable admission disposition. The catalog does not create a separate trust service.
...
| installed | yes | only through the admission predicate below | no |

An installed candidate is executable only if all are true:
...
6. The revision remains in its existing valid/active state.
...
The operation snapshot stores this observable disposition. Executor, orchestrator, rendering, generation, SDK, and every other external capability consumer must use it. No consumer reopens install metadata, re-evaluates permissions, or applies a separate trust policy.

Local, explicit-extra, and environment dispositions are assigned directly by the fixed source table above. This projection is narrow capability admission, not a universal service locator.
...
- Immediately before a security-sensitive external use:
  - capability execution verifies the candidate manifest and the resource handles consumed by the selected capability projection;
...
- A mismatch fails closed with an external-candidate-changed error.
- The operation does not retry, rediscover, update the snapshot, or admit a replacement candidate.
- Inspection-only operations that do not consume external executable resources do not perform broad reverification.

Filesystem changes become visible only in a later operation for dynamic external layers or a new bundled lifetime for bundled content.
...
Every cell receives baseline/final evidence or an explicit justified `N/A`. Runaway’s absence of stream/event/CLI/bridge surfaces remains an intentional `N/A`, not an inferred omission.
...
- Make `InstalledPackStore` emit the one installed admission projection.
...
Exit: the executable contract, catalog, lifetime, external admission, resource audit, and coverage tooling are fixture-proven.
...
- Review contract, catalog lifetime, provenance identity, external admission, documentation, resources, and ownership cumulatively.
...
`python3 -m astrid.core.pack.cli inspect <id> [--json]` exposes normalized identity, provenance class/identity, admission disposition where applicable, contribution categories, database dependencies/migrations/tables/head, documentation, runtime-resource closure, and relevant rejected/change diagnostics.
...
Exit: operational consumers agree on one snapshot and none independently reparses, rescans, or re-evaluates installed trust.
...
- Review convergence, deletion, admission, documentation, resource classification, packaging, and build-call graph.
...
- Installed inactive, unvalidated, identity-mismatched, and permission-mismatched inspect-only behavior.
...
| 5. Four-pack semantics preserved | Four-pack preservation matrix |
| 6. Owner-relative migrations and safety | WP3 SQLite matrix |
| 7. Operational agreement | WP4, Boundary C, final |
...
| 12. External capability/database policy | Admission matrix and fail-closed tests |
| 13. Golden forms | WP1 and final |
| 14. Focused/full validation | WP6 one-build flow and baseline receipt contract |
...
- **No universal service locator:** the catalog supplies immutable declarations and typed projections only; typed registries and static factories retain runtime construction ownership.
- **No duplicated DDL or mutable facts:** YAML and skill prose contain no columns, constraints, indexes, transformations, checksums, applied rows, or mutable heads. Migration SQL and SQLite remain authoritative.
...
- **No premature success:** completion is impossible while any bundled customization, documentation surface, operational consumer, packaged resource, trust decision, or database projection bypasses canonical ownership.
...
- No success with an active bypass, divergent external admission, unclassified surface, missing documentation, undeclared resource, invalid authoring exclusion, source/wheel mismatch, or source-contaminated wheel audit.

This replacement advances the complete North Star within the frozen goal. It requires no new exploration lane and no `[XHARD]` work.


## ACCEPTED WAVE 4 SYNTHESIS
...
===== RAW WAVE 4 CRITIC: .oracle/findings/settled4/authority-architecture.txt =====
1. **P0 — installed admission is not bound to the active revision at use time.**  
   §III.5 makes execution contingent on the revision being currently active/valid in `InstalledPackStore`, with matching validated provenance and permissions. §III.7’s immediate pre-use check verifies only the captured manifest and consumed resource handles; it explicitly forbids rediscovery/retry and does not revalidate active-revision state, validity, or the accepted permission receipt. Since installed revisions are selected dynamically (`astrid/core/pack/store.py:195-216`; `astrid/core/pack/discovery.py:235-257`), an active-revision switch or invalidation after snapshot capture can leave the old candidate executable for the remainder of the operation. This violates the stated single admission authority and criteria 7/12, and leaves a divergent trust decision despite the “one external admission projection” rule. The plan needs either a narrow, non-rescanning admission-state binding/check immediately before execution or evidence that `InstalledPackStore` state is immutable for the operation lifetime.

**North Star disposition:** Not CLEAN. The canonical catalog/resource architecture and typed-registry preservation align, but external execution admission is not yet durably owned by the authoritative installed-state mechanism under mutation.
0

===== RAW WAVE 4 CRITIC: .oracle/findings/settled4/sequence-proof.txt =====
1. **P0 — replacement is explicitly not `STABLE`.**  
   The plan header states `Status: complete replacement issued; not STABLE`, while accepted Wave 3 disposition explicitly requires Sol to “return exact `STABLE`” before this fresh settled-plan wave. This is a direct unresolved plan-state defect, independent of implementation quality. The architecture otherwise incorporates the accepted Wave 1–3 corrections: canonical composition, catalog lifetime, v2 contract, resource closure, provenance identity, external admission, legacy rejection, wheel isolation, and baseline receipts.

2. **P1 — Runaway round-trip prerequisite remains unresolved.**  
   Exploration evidence `E10-runaway-builtin.txt` reports the referenced `projects/runaway-piano-colour-demo` fixture is absent and requires restoration/provision before relying on the gate. The revised plan still says only: “Runaway’s missing demo input remains a deterministic temporary-project fixture repair unless custody evidence proves it is an intentional packaged asset” (§I; repeated in WP0). No custody evidence, exact fixture decision, or executable acceptance receipt is specified. Yet WP6 requires “References and runaway round trips,” and criterion 5 requires preservation of runaway behavior. This contradicts “Open planning questions: none” and leaves a material criterion-5/14 failure path.

**North Star disposition:** Conditionally aligned. The revised design rejects the named architectural anti-patterns and resolves all accepted Wave 1–3 technical objections, but it is not procedurally settled until the status is `STABLE` and the Runaway fixture prerequisite receives a deterministic disposition and evidence path.
0

===== RAW WAVE 4 CRITIC: .oracle/findings/settled4/simplicity-scope.txt =====
CLEAN

No new material simplification, defect, or unresolved investigation remains.

- **Criteria 1–4, 11–13:** strict v2 `pack.yaml`, one catalog/parser, zero-unclassified ledger, manifest-derived database composition, legacy deletion, golden fixtures, and external-database fail-closed behavior are explicit.
- **Criteria 5–7:** the four-pack preservation matrix, owner-relative migration handles, dependency ordering, shared snapshots, and operational-consumer convergence cover behavior and authority preservation.
- **Criteria 8–10:** direct structured documentation, generated `_core` census, isolated prebuilt-wheel execution, source/wheel closure, and authoring-only audits are executable requirements.
- **Criteria 14–15:** focused/full validation, one-build discipline, baseline-independence receipts, evidence matrix, and final oracle review are specified.
- **Wave 1 dispositions:** schema freeze, catalog boundary, recursive resource closure, ledger lifecycle, behavior matrix, and proportionate legacy gates are retained.
- **Wave 2 dispositions:** exact field grammar, operation snapshots, deterministic database semantics, reduced artifact cadence, and one-build packaging proof are retained.
- **Wave 3 dispositions:** fixed inventory allowlist, root-independent provenance identity, simplified lifetime/freshness model, legacy-only rejection, single admission projection, clean-wheel isolation, and baseline receipts are all explicit.
- **North Star anti-patterns:** each named anti-pattern has an explicit rejection or gate. No new service locator, lifecycle machinery, external SQL, fixed-composition disguise, compatibility path, broad scanner, cache invalidator, or redundant parser is introduced.

The runaway fixture note is bounded implementation work with a required final round-trip gate, not an open planning lane.
0

## RELEVANT ACCEPTED EXPLORATION EVIDENCE
