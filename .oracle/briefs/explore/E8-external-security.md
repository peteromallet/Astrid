Trace all external pack admission paths—local, extra roots, environment, Git installs, revision store, installed discovery, trust inspection. Identify the earliest common provenance-aware point to reject any external database contribution before registry construction or SQL access while preserving capability-only behavior.
...
Read `.oracle/agent_goal.md` completely before analysis. It is frozen and may
not be widened. This is read-only exploration: do not edit files, run mutating
commands, create branches, or dispatch models. Stay inside the canonical-pack
beta hard cut. Do not propose project locks, lifecycle machinery, dynamic
third-party database packs, a marketplace, or compatibility shims.

# Output contract

Return ranked verified facts with exact file:line evidence, affected tests,
unknowns/risks, and the simplest suggested implementation seam. State whether
each finding confirms `.oracle/plan.md` or exposes a conflict with the frozen
goal. Maximum 300 words.
