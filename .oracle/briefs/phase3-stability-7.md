...
Review the COMPLETE corrected replacement against the COMPLETE frozen goal and North Star. The prior STABLE was rejected because the plan incorrectly named a stability receipt bound to a superseded digest; verify that no stale receipt/evidence reference remains. Then verify both accepted Wave 4 corrections, every prior executable contract, all 15 done criteria, and every anti-pattern. If no material change or exploration is required, answer exactly STABLE and nothing else. Otherwise return a complete corrected replacement plan.
