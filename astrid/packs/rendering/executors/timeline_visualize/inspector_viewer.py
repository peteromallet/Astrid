...
def render_inspector(index: dict, *, columns: int = 5) -> str:
    ...
    return _DOCUMENT.replace('__STYLE__', (_ASSETS / 'inspector.css').read_text()).replace('__SCRIPT__', (_ASSETS / 'inspector.js').read_text()).replace('__COLUMNS__', str(max(1, min(12, columns)))).replace('__DATA__', payload)
...
<header><div class="topline"><div><div class="eyebrow">Timeline inspector · rendered composition</div><h1 id="title"></h1><p id="provenance" class="muted"></p></div><span id="frozen"></span></div><p id="warnings" role="status"></p>
